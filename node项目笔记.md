#项目流程
 ...
 index.js的职责：项目的统一入口
   ...
   package.json文件中的scripts中的作用,在script中配置的代码可以使用npm run,配置的key即可自动执行。
   ...
   4.富文本编辑器
    ...
    2.ueditor for nodejs
      ...
      1.请求数据的方式：
        ...
        $.ajax()最全的
      2.session和cookie(原因是http协议是无状态的,所以需要session和cookie用来维持服务端的状态,以及来标识浏览器的身份)
       ...
       3.需改
        ...
        5.连表查询
         ...
         6.删除整个表
          drop table 表名称
       作用：将客户端语法语句
    3.在mysql如何利用orm包去直接发送SQL语句到数据库服务执行
    ...
    5.利用HBULider这个开发工具的云服务将移动站点打包成apk
    6.跨域
    7.videoApp
